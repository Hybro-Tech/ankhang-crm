...
class SmartRoutingExpandJob < ApplicationJob
  ...
  def perform(contact_id = nil)
    if contact_id
      perform_for_contact(contact_id)
    else
  ...
  def perform_for_contact(contact_id)
    ...
    expand_and_schedule_next(contact)
  end

  def expand_and_schedule_next(contact)
    ...
    schedule_next_expansion(contact) if expanded
  end

  def schedule_next_expansion(contact)
    interval = contact.service_type&.visibility_expand_minutes || 2
    self.class.set(wait: interval.minutes).perform_later(contact.id)
